...
class ReplicationPutTest < Verm::TestCase
  ...
  def test_saves_files_under_requested_path
    assert_statistics_change(:put_requests => 2, :put_requests_new_file_stored => 1) do
      put_file :path => '/foo/Sn/Ui3p5f7Ht82wQtJ_PG7ostrwGnYwPZatsV0T3HBOw',
    ...
    assert_statistics_change(:put_requests => 2, :put_requests_new_file_stored => 2) do
      put_file :path => '/foo/RL/J1CmWD8NjaaUzE2Mnr-bd_8fTYcgfjC3279aQwxl9',
               :file => 'another_text_file',
               :type => 'application/octet-stream'

      put_file :path => '/different/Sn/Ui3p5f7Ht82wQtJ_PG7ostrwGnYwPZatsV0T3HBOw',
               :file => 'simple_text_file',
               :type => 'application/octet-stream'
    end
  end
  ...
  def test_saves_binary_files_without_truncation_or_miscoding
    put_file :path => '/foo/IF/A8unS2JIuR6_UZI5pZ0lxWHhfvR2ocOcRAma_lEiA',
             :file => 'binary_file',
             :type => 'application/octet-stream'
  end
  ...
  def test_saves_text_plain_files_with_suitable_extension
    put_file :path => '/foo/Sn/Ui3p5f7Ht82wQtJ_PG7ostrwGnYwPZatsV0T3HBOw.txt',
             :file => 'simple_text_file',
             :type => 'text/plain'
  end

  def test_saves_text_html_files_with_suitable_extension
    put_file :path => '/foo/Sn/Ui3p5f7Ht82wQtJ_PG7ostrwGnYwPZatsV0T3HBOw.html',
             :file => 'simple_text_file',
             :type => 'text/html'
  end

  def test_saves_image_jpeg_files_with_suitable_extension
    put_file :path => '/foo/H1/e8Qv1eQpNp-H6c5IXGExEweX4rYJlIB4IqlAofjbN.jpg',
             :file => 'jpeg',
             :type => 'image/jpeg'
  end
  ...
  def test_no_save_files_under_wrong_path
    ...
    put_file :path => '/different/path/Sn/Ui3p5f7Ht82wQtJ_PG7ostrwGnYwPZatsV0T3HBOw',
             :file => 'simple_text_file',
             :type => 'application/octet-stream'

    put_file :path => '/foo/Sn/Ui3p5f7Ht82wQtJ_PG7ostrwGnYwPZatsV0T3HBOw.ext',
             :file => 'simple_text_file',
             :type => 'application/octet-stream'
  end
end
