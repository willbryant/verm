...
void replication_backoff(struct Replicator *replicator) {
	...
	int backoff_time;
	switch (replicator->failed_push_attempts) {
		...
		case 2:
			backoff_time = BACKOFF_BASE_TIME;
			break;

		default:
			backoff_time = BACKOFF_BASE_TIME*(2 << (replicator->failed_push_attempts - 3));
...
int replication_check_response(struct Replicator *replicator, const char *location, int expected_response_code) {
	struct ReadBuffer read_buffer = EMPTY_READ_BUFFER;
	char *line, *p;
